export class Calendy {
  ...
  renderDays() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // Mostrar o ocultar el texto "multi-event"
      const multiEventClass = eventCount > 1 ? "" : "is-hidden";
      const multiEventText = `<div class="calendy-multievent ${multiEventClass}">${eventCount}</div>`;
  ...
  getEventCount(day) {
    ...
    return this.events.filter(e => e.date === date).length; // Cuenta eventos con la misma fecha
  }
